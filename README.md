...
Every exercise runs on a real product category, **Onboarding**, which includes I-9, E-Verify, Onboarding Forms, and WOTC, building specifically within the **Forms HQ** and **I-9 HQ** products: an **Onboarding Packet Agent**, an agentic assistant exposed through MCP (Model Context Protocol) tools that an HR rep can direct with a plain-language request ("Send an onboarding packet to all new hires beginning work next month"), which looks up matching new hires, confirms the match with the HR rep, and sends each of them the standard seven-document packet (EEO Self-Identification, Self-Identification of Disability, Self-Identification as a Protected Veteran, Electronic Form Delivery consent, a pay type election, WOTC, and Form I-9) to complete by Day 1. It's all for a fully fictional customer, **Meridian Logistics**, with fictional data throughout. Other Employer Services categories (offboarding, ACA, unemployment cost management, ID Watchdog) show up as secondary examples across the modules. See the data-boundary note on the home page: real work stays in enterprise Gemini; this program is where the habit gets built, safely, on personal AI accounts.
...
- `public/`: the frontend (plain HTML/CSS/JS, no build step, no framework):
  ...
  - `week.html` / `week.js`: generic module page, rendering each section as guidance, worked example, then the learner's own answer
  ...
  - `assets/fonts/`: self-hosted Open Sans woff2 files
...
| Monitoring to next Framing (closing the loop) | Week 6, section 4 |
...
Week 4 covers turning a spec into a clickable prototype outline. It also introduces [Google Stitch](https://stitch.withgoogle.com/), Google's AI UI-design tool, as a real option for PMs who have access to it: describe screens and states in plain language and Stitch generates an actual clickable mockup, which is a faster loop for visual prototyping than a text-only tool. Claude or Gemini are still the better choice when the goal is a structured written outline rather than a visual mockup.
...
Typeface is **Open Sans** throughout, self-hosted as woff2 files in `public/assets/fonts/` (see `public/styles.css` for the `@font-face` declarations and the full color token set, including the dark-mode palette). The logo lockup in the header uses `public/assets/equifax-logo.png`, extracted from the same template.
